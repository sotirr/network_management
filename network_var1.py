...
class CheckInterfaseFlagCommand(Command):
...
class GetInterfaseStatusCommand(GetIntFieldValueCommand):
    """Add intreface status."""

    flag = SIOCGIFFLAGS
    ioctl_command = GetInterfaceParamViaIoctl
    flag_checker_command = CheckInterfaseFlagCommand
    ...
    def execute(self) -> str:
        ...
        is_flag_set = CheckInterfaseFlagCommand(flags_hex, IFF_UP)
...
class GetInterfacPrefixCommand(GetIntFieldValueCommand):
    """Generate interface network prefix."""
...
class GetInterfaceMacAddressCommand(GetIntFieldValueCommand):
    """Generate mac-address in xx.xx.xx.xx.xx.xx format."""
...
INTERFACE_REPORT_FIELDS_FABRIC: dict[str, Type[GetIntFieldValueCommand]] = {
    ...
    'status': GetInterfaseStatusCommand,
